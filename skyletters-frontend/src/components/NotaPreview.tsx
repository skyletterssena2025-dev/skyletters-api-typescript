...
export default function NotaPreview({ nota, onClose }: Props) {
  const [empresa, setEmpresa] = useState<any>(null);
  const [resolucion, setResolucion] = useState<any>(null);
  const paperRef = useRef<HTMLDivElement>(null);

  const esCredito = nota.tipo === "CREDITO";
  const tipoDoc = esCredito ? "NOTA_CREDITO" : "NOTA_DEBITO";
  ...
  useEffect(() => {
    ...
    api<any[]>("/resoluciones")
      .then((list) => {
        const r = (list ?? []).find((x) => x.tipoDocumento === tipoDoc && x.estado);
        setResolucion(r ?? null);
      })
      .catch(() => {});
  }, [tipoDoc]);

  const parsed = parseDetalle(nota.detalleProducto);
  // Preferir las líneas persistidas (NotaDetalle); si no, el JSON.
  const items: Item[] =
    Array.isArray(nota.detalles) && nota.detalles.length ? nota.detalles : parsed.items;
  ...
  function printPdf() {
    ...
    if (!w) return;
    w.document.write(
      `<html><head><title>${titulo} ${nota.numero}</title><style>${PRINT_CSS}</style></head><body>${node.outerHTML}</body></html>`,
    );
    w.document.close();
    w.focus();
  ...
  return (
    <Modal
      title={`Nota ${esCredito ? "crédito" : "débito"} N° ${nota.numero}`}
      onClose={onClose}
      wide
    >
        ...
        <div className="invoice-paper" ref={paperRef}>
          <div className="inv-top">
            ...
            <div className="inv-doc">
              ...
              <div className="num">N° {nota.numero}</div>
              <div className="meta">{fdate(nota.fecha)}</div>
              <div className="meta">Modifica la factura N° {nota.factura?.numeroFactura}</div>
              <div className="meta">Motivo: {nota.motivo}</div>
            </div>
          </div>
